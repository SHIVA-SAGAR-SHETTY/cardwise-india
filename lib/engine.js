...
function eligible(card, p) {
  if (p.credit_history === "none") return card.best_for.includes("no-credit-history") || (card.min_income_monthly <= p.income && card.tier === "entry");
  if (p.credit_history === "new" && card.tier === "super-premium") return false;
  if (p.income && card.min_income_monthly > p.income * 1.1) return false;
...
Pick the best 3 cards for this user ONLY from the shortlist (use the exact "id"). Consider: where they spend most, fee vs value, fee waivers, eligibility (income, credit history), their priorities, lounge/travel needs, forex, and caps.
